...
После этого бот поддерживает команды:

- `/help` — список команд;
- `/tickets` — список доступных обращений;
- `/ticket ID` — детали обращения;
- `/new тема | описание | приоритет` — создание обращения пользователем;
- `/reply ID текст` — сообщение в переписку по обращению;
- `/status ID RESOLVED` — смена статуса исполнителем.
