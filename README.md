...
Email-уведомления поддерживают пользовательское согласие: в разделе «Уведомления» пользователь может разрешить или запретить отправку писем. Для безопасной демонстрации без собственного домена поддержан провайдер `mailtrap`: письма отправляются в Mailtrap Sandbox через HTTPS API и не уходят случайным реальным получателям. Также пользователь может подключить Telegram-уведомления без ручного ввода `chat_id`: система выдаёт одноразовую ссылку на бота и автоматически сохраняет чат после команды `/start`.
...
- внешние каналы уведомлений: Mailtrap Sandbox для тестовых email и Telegram Bot API для сообщений пользователям;
- Telegram-бот с кнопочным интерфейсом: просмотр обращений, пошаговое создание обращения, ответы в переписку и смена статуса исполнителем;
